...
import {removeOrganizer} from "@/db/actions/removeOrganizer";
...
export default function Organizers() {
    ...
    const handleDeleteEvent = async (orgEmail: string, orgName: string) => {
        if (!confirm(`Sei sicuro di voler eliminare l'organizzatore "${orgName}"? Questa operazione è irreversibile.`)) {
            return;
        }
        try {
            await removeOrganizer(orgEmail);
        } catch (error) {
            console.error(`Errore inaspettato durante l'eliminazione organizzatore:`, error);
        }finally {
            fetchOrganizer().then(res => {
                console.log(res);
                setOrganizerList(res || [])});
                    ...
                    <table className="w-full text-left table-auto min-w-max">
                        ...
                        {organizerList.map((organizer, index) => (
                            <tr className="hover:bg-slate-50 border-b border-slate-200" key={index}>
                                ...
                                <td className="p-4 py-5">
                                    <button
                                        onClick={() => handleDeleteEvent(organizer.user["email"], organizer.companyName)}
